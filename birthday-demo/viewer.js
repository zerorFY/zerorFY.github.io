(() => {
  ...
  const photoPool = window.BirthdayPhotoPool;
  ...
  async function reloadCloudPhotos(){
  ...
  function scheduleReload(){
    ...
    reloadTimer=setTimeout(()=>reloadCloudPhotos().catch(err=>{console.error(err);badge.textContent='PHOTO WALL OFFLINE'}),120);
  }

  async function hydrateCloud(){
    if(!api?.ready){ badge.textContent='DEMO PHOTO WALL'; return; }
    try{
      await reloadCloudPhotos();
      badge.textContent='LIVE PHOTO WALL'; badge.classList.add('live');
      api.subscribe(handleInsert, scheduleReload);
